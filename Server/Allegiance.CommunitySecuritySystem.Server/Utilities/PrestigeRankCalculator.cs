...
{
    public class PrestigeRankCalculator
    {
        private const int TeamMemberBonusPoints = 2;
        private const int WinningCommanderBonusPoints = 100;
        private const int LosingCommanderBonusPoints = 25;
        private const int BonusPointsPerMinute = 1;
        ...
        private bool IsGameScorable(Game game)
        {
            ...
            // Did each team have 2 members who played at least 5 minutes?
            foreach (var gameTeam in game.GameTeams)
            {
                if(gameTeam.GameTeamMembers.Where(p => p.GameTeamMemberDuration >= 300).Count() < 2)
                    return false;
            }
